...
def main():
    ...
    parser.add_argument("--pretrained-encoder", type=str, default=None,
                        help="Path to pretrained encoder weights (pretrained_encoder.pt)")
    ...
    if args.pretrained_encoder:
        ...
        pretrained = torch.load(args.pretrained_encoder, map_location=DEVICE, weights_only=True)
        ...
        print(f"  Loaded {loaded} pretrained tensors")
        ...
        use_disc_lr = True
        print(f"  Discriminative LR: encoder={len(encoder_params)} params, "
              f"heads={len(head_params)} params")
    else:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for i, pg in enumerate(optimizer.param_groups):
            if use_disc_lr and i == 0:  # encoder group
                pg["lr"] = lr * 0.25
            else:
                pg["lr"] = lr
